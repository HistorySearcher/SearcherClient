// 드랍박스 SearchProject-완성 background.js 백업
chrome.browserAction.onClicked.addListener(function (tab) {
    chrome.tabs.create({
        url: "http://112.108.40.87:8080/Lucene4.3-Server-rev02/ChromeHistory_input.jsp?user="+localStorage.getItem("username")
    }
    );
...
function onUserInfoFetched(error,status ,response) {
   ...
   user_info.id;
    ...
    chrome.storage.local.set({'google_user_id':userId});
...
//    chrome.tabs.executeScript(tabId, {file: 'oauth2.js'},function(){
//        getUserInfo(true);
//
//    });
    ...
    if (changeInfo.status === 'complete')
    {
        ...
        ai=0;
            ...
            chrome.tabs.executeScript(tabId, {file: 'onComplete.js'},function(){
                chrome.extension.onMessage.addListener(function(request, sender) {
                    if (request.action == "geturl") {
                        curl = request.source;
                    }
                    if (request.action == "geturlcut") {
                        urlcut = request.source;
                    }
                    ...
                    if(curl!=="" && title!=="" && urlcut!=="" && context!=="")
                        senddata(userId,context,title,curl,urlcut);
...
function senddata(s1, s2,  s3, s4, s5){

    if((s4!=="http://112.108.40.87:8080/Lucene4.3-Server-rev02/index.jsp")&&(s5!=="http://search.zum.com/search.zum")&&(s5!=="http://search.daum.net/nate")&&(s5!=="https://www.google.co.kr/webhp")&&(s5!=="http://search.naver.com/search.naver")&&(s5!=="http://search.daum.net/search"))
    {
        //alert("send start");
        jQuery.ajax({
            type:"POST",
            ...
            data: {user:s1,content:s2,title:s3,url:s4},
...
/*
 ...
 chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
 if (changeInfo.status === 'complete') {
 chrome.tabs.executeScript(tabId, {file: 'jquery-2.0.3.js'}, function () {
 // chrome.tabs.executeScript(tabId, {file: 'onComplete.js'});
 ...
 localStorage.setItem("username",dateValue );
 //   localStorage.clear();
 alert("else   "+localStorage.getItem("username"));
 // localtest = localStorage.getItem("username");
